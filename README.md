...
- **rebase-mrs**: scans my open GitLab MRs across all projects, server-side rebases the unapproved-but-behind ones, and flags approved-but-stale ones plus rebase conflicts. Schedule via `/loop 1h /rebase-mrs --working-hours-only` (inside Claude Code) or launchd (`skills/rebase-mrs/launchd/install.sh`, true set-and-forget). Trigger manually: `/rebase-mrs` (add `--dry-run` to preview).
